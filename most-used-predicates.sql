with tb_settings as (
  select cast('2022-10-17' as timestamp) as start_date,
         cast('2022-10-20' as timestamp) as end_date,
)
, tb_org as (
...
select table_name, predicate_column, table_used_cnt, predicate_used_cnt, predicate_used_cnt / table_used_cnt as column_used_ratio 
  from tb_table_cnt join tb_column_cnt using (table_name)
 order by 5,3,4 desc
